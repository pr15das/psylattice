...
type AdminBillingPlanPayload = {
  ...
  last_provider_event:
    | "admin.complimentary_plan_removed"
    | "admin.complimentary_plan_granted";
  ...
  try {
    ...
    const durationDays = Math.max(
      ...
      Math.min(
        3650,
        Math.floor(
          Number(body.durationDays || (plan === "pro-annual" ? 365 : 30)),
        ),
      ),
    );
    ...
    if (!["free", "pro-monthly", "pro-annual"].includes(plan)) {
      return NextResponse.json(
        { ok: false, error: "Choose a supported plan." },
        { status: 400 },
      );
    }
    ...
    if (reason.length < 4) {
      return NextResponse.json(
        { ok: false, error: "Add a short reason for this admin change." },
        { status: 400 },
      );
    }
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "This user has a Razorpay subscription. Manage or cancel the paid subscription before applying a manual plan override.",
        },
        { status: 409 },
      );
    }
    ...
    let next: AdminBillingPlanPayload;
    ...
    if (plan === "free") {
      ...
      const payload: AdminBillingPlanPayload = {
        user_id: userId,
        plan_tier: plan,
        // Existing entitlement logic treats completed + a future period end as current.
    ...
    return NextResponse.json({
      ok: true,
      plan,
      endsAt: plan === "free" ? null : end.toISOString(),
    });
  } catch (error) {
