...
function WorkshopTeaserIllustration() {
  return (
    <svg
      viewBox="0 0 760 460"
      role="img"
      aria-label="Illustrated four-week PsyLattice workshop pathway"
    ...
    >
      <defs>
        ...
        <filter id="workshop-teaser-shadow" x="-30%" y="-30%" width="160%" height="160%">
          <feDropShadow dx="0" dy="18" stdDeviation="20" floodColor="#0f172a" floodOpacity=".12" />
        </filter>
      </defs>

      <rect x="20" y="20" width="720" height="420" rx="44" fill="url(#workshop-teaser-bg)" />
      <circle cx="650" cy="80" r="44" fill="#cffafe" opacity=".8" />
      <circle cx="98" cy="360" r="54" fill="#ede9fe" opacity=".9" />
      ...
      <path
        d="M164 232 C225 168 286 296 347 222 C407 151 470 289 586 202"
        fill="none"
        stroke="url(#workshop-teaser-line)"
        strokeWidth="7"
        strokeLinecap="round"
      />
    ...
    <section className="border-y border-slate-200 bg-white px-5 py-20 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-[1320px] gap-10 lg:grid-cols-[.82fr_1.18fr] lg:items-center">
        <div>
          ...
          <h2 className="mt-5 max-w-xl text-4xl font-semibold tracking-[-.04em] text-slate-950 sm:text-[48px]">
            Learn modern research by building it.
          </h2>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            ...
            <span className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-[#f5f9f8] px-3 py-2 text-xs font-semibold text-slate-700">
              ...
              1 hour · every weekend · 4 sessions
            </span>
          </div>

          <p className="mt-5 text-xs leading-5 text-slate-500">
            Exact timings, detailed session information and the full brochure are shared privately with registered participants.
          </p>

          <Link
            href="/workshops"
            className="mt-7 inline-flex items-center gap-2 rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:-translate-y-px hover:bg-cyan-950"
          >
            Explore workshops
        ...
        <div className="relative">
          ...
          <div className="relative overflow-hidden rounded-[34px] border border-slate-200 bg-[#f8fbfb] shadow-[0_24px_70px_rgba(15,23,42,.09)]">
            <WorkshopTeaserIllustration />
          </div>
        </div>
      </div>
    </section>
