...
const SAFE_MESSAGES: Record<string, string> = {
  ...
  ACCOUNT_RESTRICTED: "This PsyLattice account is restricted.",
...
export function isMobileAccountRestricted(
...
export async function authenticateMobileRequest(
  ...
  // Mobile API routes are outside the browser workspace proxy. Apply the
  // current-main account restriction model here so suspended, banned, or
  // deleting accounts cannot keep using a valid bearer token on mobile.
  ...
  if (isMobileAccountRestricted(access)) {
    return mobileError("ACCOUNT_RESTRICTED", 403);
  }
