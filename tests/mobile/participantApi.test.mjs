...
test("mobile bearer auth preserves current-main account restrictions", () => {
  const now = Date.parse("2026-09-17T00:00:00Z");
  assert.equal(isMobileAccountRestricted(null, now), false);
  assert.equal(isMobileAccountRestricted({ status: "active" }, now), false);
  assert.equal(isMobileAccountRestricted({ status: "banned" }, now), true);
  assert.equal(isMobileAccountRestricted({ status: "deleting" }, now), true);
  assert.equal(isMobileAccountRestricted({
    status: "suspended",
    suspended_until: "2026-09-18T00:00:00Z",
  }, now), true);
  assert.equal(isMobileAccountRestricted({
    status: "suspended",
    suspended_until: "2026-09-16T00:00:00Z",
  }, now), false);
});
