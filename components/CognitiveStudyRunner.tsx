...
export default function CognitiveStudyRunner({
    ...
    if (block.block_type === "experimental") {
      for (const mode of modes) {
        ...
        while (span <= settings.max_span) {
          ...
          if (correctAtSpan >= settings.pass_required) span += 1;
          else break;
        }
      }
    }
  ...
  async function runMentalRotationBlock(
    ...
    for (let localIndex = 0; localIndex < plans.length; localIndex += 1) {
      if (cancelledRef.current) throw new Error("Study cancelled");
  ...
  async function completeSession(id: string, trialResults: PreviewTrialResult[], finalSummary: Record<string, any>, device: Record<string, any>) {
    ...
    const timingQuality = {
      engine: mentalRotationRuntime ? "psylattice_mental_rotation_study_v1" : bartRuntime ? "psylattice_bart_study_v1" : cardSortRuntime ? "psylattice_card_sorting_study_v1" : corsiRuntime ? "psylattice_corsi_study_v1" : stopSignalRuntime ? "psylattice_stop_signal_study_v1" : "psylattice_browser_study_v1_frame_calibrated",
    ...
    try {
      ...
      const totalPlanned = blocks.reduce((sum, block) => {
        ...
        if (corsiRuntime && block.block_type === "practice") {
          return sum + corsiSettings.practice_trials * corsiModes(corsiSettings).length * Math.max(1, block.repeat_count);
        }
        if (corsiRuntime && block.block_type === "experimental") {
          return sum + (corsiSettings.max_span - corsiSettings.start_span + 1) * corsiSettings.trials_per_span * corsiModes(corsiSettings).length * Math.max(1, block.repeat_count);
        }
      ...
      for (const block of [...blocks].sort((a, b) => a.position - b.position)) {
        ...
        for (let repeat = 1; repeat <= Math.max(1, block.repeat_count); repeat += 1) {
          ...
          if (corsiRuntime && (block.block_type === "practice" || block.block_type === "experimental")) {
            const corsiBlockResults = await runCorsiBlock(block, runSeed, globalIndex, repeat, corsiSettings);
      ...
      const finalSummary = mentalRotationSummary
        ...
        : bartSummary
        ? { ...genericSummary, paradigm: "bart", adjusted_mean_pumps: bartSummary.adjusted_mean_pumps, bart: bartSummary }
        : cardSortSummary
        ? {
            ...genericSummary,
            paradigm: "card_sorting",
                        ...
                        <button
                          ...
                          className={`relative flex aspect-[4/3] items-center justify-center rounded-[24px] border bg-white p-3 transition ${
                            selected
                              ...
                              : display.interactive
                                ? "border-slate-300 shadow-[0_9px_22px_rgba(15,23,42,0.10)] active:scale-[0.99]"
              ...
              {responseOptions.length > 0 && display.kind !== "message" && display.kind !== "corsi" && display.kind !== "card_sort" && display.kind !== "bart" && (
                <div className="mt-10 flex flex-wrap justify-center gap-3">
                  {responseOptions.map((option) => (
                    <button key={option} type="button" onPointerDown={() => responseHandlerRef.current?.(option, performance.now())} className="min-w-20 rounded-full border border-slate-300/80 bg-white px-5 py-3 text-sm font-semibold text-slate-800 shadow-sm">{option === "space" ? "Space" : option === "same" ? "Same" : option === "mirrored" ? "Mirrored" : option}</button>
                  ))}
                </div>
              )}
